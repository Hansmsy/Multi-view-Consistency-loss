...
class ViewLoss(BaseLoss):
    def __init__(self, margin, loss_term_weight=1.0, gamma=0.6):
        super(ViewLoss, self).__init__(loss_term_weight)
        self.margin = margin
        self.gamma = gamma
    ...
    def forward(self, embeddings, labels):
        ...
        ap_dist, an_dist = self.Convert2Triplets(labels, ref_label, dist, dist)
    ...
    def Convert2Triplets(self, row_labels, clo_label, dist, dist_weight):
        """
            row_labels: tensor with size [n_r]
            clo_label : tensor with size [n_c]
        """
        dist = (dist_weight ** self.gamma) * dist
